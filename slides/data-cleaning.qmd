...
```{r echo = TRUE, eval = FALSE}
mutate(penguins,
      bill_stuff = bill_length_mm / bill_depth_mm) 
```
...
```{r}
penguins %>%
mutate( bill_stuff = bill_length_mm / bill_depth_mm) |>
      mutate(across(contains("bill"), ~as.character(.))) |>
      select(contains("bill")) |>
      head(5) |>
      bind_rows(tibble(bill_length_mm = "...", bill_depth_mm = "...", bill_stuff = "...")) |>
      knitr::kable(format = "html")
```
...
summarise(group_by(filter(penguins,sex == "female"),
