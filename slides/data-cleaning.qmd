...
# Cleaning Data in the Tidyverse {background-color="`r clrs[6]`" .white}
...
- One of the most furious debates in R is base R versus tidyverse. 
  - `data.table` vs `dplyr` is also a debate you sometimes see

### The Pros of the Tidyverse
::: incremental
...
# Dplyr {background-color="`r clrs[6]`" .white}
...
# filter() {background-color="`r clrs[6]`" .white}
...
```{r eval=FALSE, echo = TRUE}
  ...
  filter(starwars, homeworld %in% c("Naboo",
   "Tatooine",
    "Coruscant")
         & !grepl(c("ars"),name))
...
starwars |> 
  ...
  filter(homeworld %in% c("Naboo", "Tatooine", "Coruscant") & !grepl(c("ars"),name)) |>
    head(9) |> 
...
We are likely to encounter some sets of errors. Luckily Dplyr has finally built in a friendly error message with when your tests are misspecified. The equals one is one that we all make. Which will just now throw an error. What your are likely to encounter is that you have done a test and your object is now blank. It will return your something back to you but it is blank. All this means is that something is wrong in one of your tests. R will evaluate the test but something in your test does not exist in your data. So it evaluates your test correctly but what you supplied it is not in the data so R does it but there is nothing there for it to return true
...
# Select() {background-color="`r clrs[6]`" .white}
...
# Doing More Than One Thing at a Time {background-color="`r clrs[6]`" .white}
...
# Mutate {background-color="`r clrs[6]`" .white}
...
<<<<<<< HEAD
# Group_by() and Summarize() {background-color="`r clrs[6]`" .white}
=======
...
# Tidyr {background-color="`r clrs[6]`" .white}
...
# Getting up and Running Using helpers  {background-color="`r clrs[6]`" .white}
...
select(-year) |>
group_by(species) |>
summarise(across(where(is.numeric), c(Mean = mean, Min = min, Max = max), na.rm = TRUE), .names = "{.cols}_{.fn}")
