...
install.packages(c("tidyverse", "palmerpenguins"), dependencies = TRUE)
...
:::{.notes}
The entire tidyverse paradigm is sort of centered around this idea and how to make manipulating data a lot less painful. 
...
What this means in practice is that the packages in the tidyverse has a very similar syntax which makes working with each package and function relatively painless

If you are intersted in learning SQL lots of the stuff that we learn today translate nicely over to SQL. So much so that there is a package called dbplyr which will actually translate your dplyr code to sql code!


There is a massive debate between tidyverse vs data.table and base r versus the tidyverse. Both sides have their merit but generally the debate gets a bit heated. Each have their place in your workflow! Don't esque one solution for another if it gets the job done right! The tidyverse is great it helped me learn R a lot quicker. But it has its issues and in some cases is not the correct tool for the job. Thats why I included a data.table workshop slide. 
:::
...
::: {.notes}
These are the packages that are considered a part of the tidyerse. The ones that are loaded when you do library tidyverse are ggplot tibble tidyr readr purr dplyr stringr and forcats. Magrittr is also loaded.
...
tidyr: helps us make data long or short and make stuff. 
...
:::{.notes}
Dplyr tries to be really human readable so we refer to each function as verbs. I will
...
I will only partially cover select because it is a pretty intuitive verb. Just give it the name of our columns. But see that the first argument we feed our verb is data and in this case the thing that verb does is just select the colunn name we want. 
...
## Chaining together multiple things 

- To get a subset of homeworld we chain together multiple `|`
together
...
set membership works really here too!
...
Here I am just asking filter to return all the values in the data that are missing. 
...
- Return a data dataset that only has data for `Dream` 
...
starwars |> 
  select(name,  -eye_color, ends_with("color")) |> 
  head(4) |> 
...
```{r eval=FALSE, echo = TRUE}
# base R regex
filter(starwars, grepl("Skywalker", name)|  # base r version
           grepl("Palp", name) |
           grepl("Obi", name))
# tidyverse regex
filter(starwars, str_detect(name, "Skywalker") | 
