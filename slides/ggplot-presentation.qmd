...
```{r viridis-color-show-og-pal, echo = TRUE}
ggplot(gapminder, aes(x = gdpPercap, y = lifeExp, size = pop,
                      color = continent)) +
  geom_point() +
  labs(x = "Income", y = "Life Expectancy") +
...
```{r coord-flip-show-option, echo = TRUE}
ggplot(gapminder, aes(x = gdpPercap, y = lifeExp)) +
  geom_point() +
  scale_x_continuous(limits = c(0, 30000)) +
  coord_flip() +
...
```{r facet-wrap-two-col, echo = TRUE}
...
```{r gall-peters-projection,  eval=FALSE, echo = TRUE}
ggplot() +
  geom_sf(data = world_map_ne) + 
