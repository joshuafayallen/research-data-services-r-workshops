...
```{r tidyverse-install, echo = TRUE, eval = FALSE}
...
```{r ggplot-install, echo = TRUE, eval=FALSE}
...
```{r  pacaman-install, eval=FALSE, echo =TRUE}
...
There are some other packages like geom_map and ggpmap. They still work but sf(simple features) is really great and has a ton of support and resources. 
