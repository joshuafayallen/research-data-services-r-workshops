...
```{r gapminder-blue-correct, echo = TRUE}
#| code-line-numbers: "|5"
#| output-location: column
ggplot() + 
  geom_point(data = gapminder,
...
ggplot() +
  geom_boxplot(gapminder,
   mapping = aes(x = continent, # ggplot got map so I had to add mapping
          y = lifeExp))
```
...
```{r reference-plot, echo=FALSE, fig.width = 7, }
...
:::{.notes}
Lots of organizations use ggplot. What this means for you is that you can find themes that 
...
breweries_geocode = breweries_I_visit |> 
  geocode(address, geocode_combine = c("osm", "arcgis")) ## backup if one service fails 
