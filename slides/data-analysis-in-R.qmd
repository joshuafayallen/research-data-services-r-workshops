...
:::{.callout-tip}
[interflex](https://yiqingxu.org/packages/interflex/) is also good a package
and implements diagnostics for violations of the assumptions underlying interaction terms and a way to interpret the effects. [inters](https://github.com/mattblackwell/inters) also has an interesting implementation where they use LASSO
:::
:::{.notes}
We may be interested in what happens when we shift the values of our continous variable. One awesome way to do this is through our marginal effects package. In this case we can look at the effect as bill_length_mm and the cco and cme are essentially looking at when the factor is on or off. So in this case this is just showing the effect when penguins take on the value of male
...
peng_stand = lm(body_mass_g ~  bill_length_mm + flipper_length_mm + factor(species) + factor(island) , data = peng_sans_miss_tidy)
...
- I just choose bootstrap. Do what makes sense for your goals 
